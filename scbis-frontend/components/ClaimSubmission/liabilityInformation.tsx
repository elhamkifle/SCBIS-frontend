...
export default function LiabilityInformation() {
  ...
  const [otherInsuredStatus, setOtherInsuredStatus] = useState('');
    ...
    <div className="max-w-5xl mx-auto p-6 bg-white">
        ...
        <div className="flex flex-wrap lg:flex-nowrap gap-4">
          {['Myself', 'The other person'].map((option) => (
            <label key={option} className="flex items-center">
              <input type="radio" name="responsibleParty" value={option} onChange={() => setResponsibleParty(option)} checked={responsibleParty === option} className="mr-2" />
              {option}
            </label>
          ))}
        </div>
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap lg:flex-nowrap gap-4">
          {['I dont know', 'Yes , they are'].map((option) => (
            <label key={option} className="flex items-center">
              <input type="radio" name="otherInsuredStatus" value={option} onChange={() => setOtherInsuredStatus(option)} checked={otherInsuredStatus === option} className="mr-2" />
              {option}
            </label>
          ))}
        </div>
      ...
      <div className="mt-4">
        ...
        <div className="flex flex-wrap lg:flex-nowrap gap-4">
          {['Yes', 'No'].map((option) => (
            <label key={option} className="flex items-center">
              <input type="radio" name="policeInvolved" value={option} onChange={() => setPoliceInvolved(option)} checked={policeInvolved === option} className="mr-2" />
              {option}
            </label>
          ))}
        </div>
