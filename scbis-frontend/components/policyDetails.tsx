...
export default function PolicyDetails() {
  ...
  const isPending = selectedPolicy.status.value == "pending";
  const isActive = selectedPolicy.status.value == "active";
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 text-gray-800">
      <div className="max-w-3xl mx-auto px-4 pt-8 pb-12">
        ...
        {isPending ? (
          <div className="bg-yellow-50 border-l-4 border-yellow-500 text-yellow-900 p-6 rounded-lg shadow-md">
            <h2 className="text-2xl font-semibold mb-2">Your policy is currently pending</h2>
            <p className="mb-1">We have received your request and it is under review.</p>
            <p className="mb-1">You will receive a notification once it’s approved and ready for payment.</p>
            <p className="text-sm text-yellow-700 mt-4 italic">Thank you for your patience.</p>
          </div>
        ) : isActive ? (
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-blue-100 transform hover:scale-[1.01] transition-transform duration-200">
            ...
            <div className="bg-blue-50 p-4 rounded-lg mb-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  <div className="font-bold text-blue-700">Policy Duration</div>
                  <div className="text-gray-700">{selectedPolicy.duration} days</div>
                </div>
              </div>
            </div>
          </div>
        ) : (
          <div className="text-center text-red-600 font-semibold">Unsupported policy status.</div>
        )}
        ...
        <div className="flex justify-center mt-8">
          <button
            type="button"
            className="bg-[#3AA4FF] text-white px-6 py-3 rounded-lg shadow hover:bg-blue-600 transition duration-200"
